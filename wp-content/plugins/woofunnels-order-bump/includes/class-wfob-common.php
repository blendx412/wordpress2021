...
class WFOB_Common {
		...
		if ( $q->have_posts() ) {
			while ( $q->have_posts() ) {
				...
				$row_actions['delete']         = array(
					...
					'text'   => __( 'Delete Permanently', 'woofunnels-order-bump' ),
	...
	public static function get_sidebar_menu() {
		...
		$sidebar_menu = array(
			...
			'20' => array(
				'icon' => WFOB_ADMIN_PATH . '/assets/img/product.png',
				'name' => __( 'Products', 'woofunnels-order-bump' ),
				'key'  => 'products',
			),
			'30' => array(
				'icon' => WFOB_ADMIN_PATH . '/assets/img/Icon_design_for_order_bump.png',
				'name' => __( 'Design', 'woofunnels-order-bump' ),
				'key'  => 'design',
			),
			'40' => array(
				'icon' => WFOB_ADMIN_PATH . '/assets/img/settings.png',
	...
	public static function get_builder_localization() {
		...
		$data['global'] = [
			...
			'add_bump'            => [
				...
				'edit_order_bump'   => __( 'Edit Order bump', 'woofunnels-order-bump' ),
				'new_bump'          => __( 'Add New Order Bump', 'woofunnels-order-bump' ),
			],
		...
		$data['settings'] = [
			...
			'priority_hint'             => __( 'There may be a chance that Order Bump can be set up in a way that two Order Bump can trigger. In such cases, Prioirty is used to determine which Order Bump will display. Priority Number 1 is considered highest.', 'woofunnels-order-bump' ),
			'priority_label'            => __( 'Priority', 'woofunnels-order-bump' ),
			'order_bump_position_label' => __( 'Position', 'woofunnels-order-bump' ),
		...
		$data['design']   = [
			...
			'additional_settings'                => __( 'Additional Settings', 'woofunnels-order-bump' ),
			'header_enable_pointing_arrow'       => __( 'Pointing  Arrow', 'woofunnels-order-bump' ),
			...
			'content_variation_link_color'       => __( 'Variation Link Color', 'woofunnels-order-bump' ),
			'content_variation_link_hover_color' => __( 'Variation Link Hover Color', 'woofunnels-order-bump' ),
			'featured_image_border_label'        => __( 'Featured Image', 'woofunnels-order-bump' ),
			'border'                             => __( 'Border', 'woofunnels-order-bump' ),
			'padding'                            => __( 'Padding', 'woofunnels-order-bump' ),
			'background'                         => __( 'Background Color', 'woofunnels-order-bump' ),
			'hover_background'                   => __( 'Background hover color', 'woofunnels-order-bump' ),
			'color'                              => __( 'Color', 'woofunnels-order-bump' ),
			'font_size'                          => __( 'Font Size', 'woofunnels-order-bump' ),
			'hover_color'                        => __( 'Hover Color', 'woofunnels-order-bump' ),
			...
			'point_animation'                    => __( 'Animated', 'woofunnels-order-bump' ),
			'error_color'                        => __( 'Error Color', 'woofunnels-order-bump' ),
			'layout_change'                      => [
				'title'             => __( 'Are you sure want to change layout', 'woofunnels-order-bump' ),
				'text'              => __( 'The current style settings would be changed as per new skin.', 'woofunnels-order-bump' ),
				'confirmButtonText' => __( 'Yes!', 'woofunnels-order-bump' ),
			],
			'border_type'                        => [
				[
					'id'   => 'solid',
	...
	public static function get_design_data( $wfob_id ) {
		...
		self::$design_model_data   = self::get_design_data_meta( $wfob_id );
		if ( ! is_array( self::$design_model_data ) || count( self::$design_model_data ) == 0 ) {
			self::$design_model_data           = $design_default_model_data[ self::$design_default_layout ];
			self::$design_model_data['layout'] = self::$design_default_layout;
		}

		$data['design_default_model_data'] = $design_default_model_data;
		$data['product_field']             = self::get_design_product_fields( $wfob_id );
		$data['model']                     = self::$design_model_data;
	...
	public static function get_design_product_fields( $wfob_id ) {
		...
		if ( is_array( $products ) && count( $products ) > 0 ) {
			foreach ( $products as $key => $product ) {
				$fields[] = [
					'type'  => 'label',
					'label' => __( $product['title'], 'woofunnels-order-bump' ),
				...
				$fields[] = [
					...
					'model'        => 'product_' . $key . '_title',
					'styleClasses' => 'hint_custom_cls_wrap',
					'hint'         => 'Use merge tag {{product_name}} to show product name dynamically. Useful in case if OrderBump is a variable product',
				];
				...
				$fields[] = [
					...
					'rows'         => 3,
					'model'        => 'product_' . $key . '_description',
					'styleClasses' => 'hint_custom_cls_wrap',
					'hint'         => 'Use merge tag {{subscription_summary}} to show pricing details of subscription products',
				];
				$fields[] = [
					'type'    => 'switch',
					'label'   => __( 'Featured Image', 'woofunnels-order-bump' ),
					'model'   => 'product_' . $key . '_featured_image',
					'textOn'  => __( 'Acive', 'woofunnels-order-bump' ),
					'textOff' => __( 'InAcive', 'woofunnels-order-bump' ),
				];

				if ( ! isset( self::$design_model_data[ 'product_' . $key . '_title' ] ) ) {
					self::$design_model_data[ 'product_' . $key . '_title' ] = $product['title'];
					self::$design_model_data[ 'product_' . $key . '_title' ] = __( "<span style='color:#ff0000'>Yes!</span> Add ", 'woofunnels-order-bump' ) . '{{product_name}}' . __( ' to my order', 'woofunnels-order-bump' );
				}

				if ( ! isset( self::$design_model_data[ 'product_' . $key . '_description' ] ) ) {
					self::$design_model_data[ 'product_' . $key . '_description' ] = $product['description'];
				}
				if ( ! isset( self::$design_model_data[ 'product_' . $key . '_featured_image' ] ) ) {
					self::$design_model_data[ 'product_' . $key . '_featured_image' ] = true;
				}
			}
		}
		if ( count( $fields ) > 0 ) {
	...
	public static function set_product_price( $pro, $data ) {
		...
		$discount_data = [
			...
			'wfob_discount_amount' => $data['discount_amount'],
	...
	public static function decode_merge_tags( $content, $price_data, $pro = false, $product_data = [], $cart_item = [], $cart_item_key = '' ) {

		return WFOB_Product_Switcher_Merge_Tags::maybe_parse_merge_tags( $content, $price_data, $pro, $product_data, $cart_item, $cart_item_key );
	}
	...
	public static function remove_actions( $hook, $cls, $function = '' ) {
		...
		add_action( 'woocommerce_checkout_update_order_review', function () {
			if ( isset( $_REQUEST['wc-ajax'] ) && in_array( $_REQUEST['wc-ajax'], [ 'wfob_add_order_bump', 'wfob_remove_order_bump' ] ) ) {
