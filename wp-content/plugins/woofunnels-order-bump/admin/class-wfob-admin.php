...
class WFOB_Admin {
	...
	public function __construct() {
		...
		add_action( 'manage_shop_order_posts_custom_column', [ $this, 'show_bump_total_content' ], 13, 2 );
	...
	public function admin_enqueue_assets() {
		...
		if ( WFOB_Common::is_load_admin_assets( 'builder' ) ) {
			wp_enqueue_style( 'wfob-funnel-bg', $this->admin_url . '/assets/css/wfob-funnel-bg.min.css', array(), WFOB_VERSION_DEV );
		...
		if ( WFOB_Common::is_load_admin_assets( 'all' ) ) {
			...
			wp_enqueue_style( 'wfob-admin', $this->admin_url . '/assets/css/wfob-admin.min.css', array(), WFOB_VERSION_DEV );
			...
			wp_enqueue_script( 'wfob', $this->admin_url . '/assets/js/wfob.min.js', array( 'wfob-gl', 'jquery', 'underscore', 'backbone' ), WFOB_VERSION_DEV );
	...
	public function show_bump_total_content( $column_name, $post_ID ) {
		if ( $column_name == 'order_total' ) {
			...
			if ( $line_total > 0 ) {
				...
				echo '<br>OrderBump: ' . wc_price( $line_total );
			}
		}
	}
