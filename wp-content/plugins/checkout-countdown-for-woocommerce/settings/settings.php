...
if ( ! class_exists( 'Checkout_Countdown_Main' ) ) :
	...
	class Checkout_Countdown_Main {
		...
		function __construct( $setup ) {
			...
			require_once dirname( $this->setup['path'] ) . '/settings/banana-framework/class-settings-api.php';
		...
		public function load_classes() {
			...
			if ( class_exists( 'Checkout_Countdown_Settings_API_PRO' ) && class_exists( 'Checkout_Countdown_Settings_API' ) ) {

				$this->settings_api = new Checkout_Countdown_Settings_API_PRO( $this->setup );

			} elseif ( class_exists( 'Checkout_Countdown_Settings_API' ) ) {
				// Use this if plugin is free.
				$this->settings_api = new Checkout_Countdown_Settings_API( $this->setup );

			} else {
				error_log( print_r( new WP_Error( 'error', 'No class found for ' . $this->setup['name'] ), true ) );
				return;
			}
		...
		public function load_textdomain() {
			load_plugin_textdomain( $this->setup['text-domain'], false, basename( dirname( $this->setup['path'] ) ) . '/languages' );
		}
		...
		public function get_settings_sections() {
			...
			$sections = $this->extendable( $sections, 'extend_settings_sections' );
		...
		public function get_settings_fields() {
			...
			$settings_fields['ccfwoo_general_section'] = array(
				...
				array(
					...
					'desc'        => __(
						'<p>
					<b>Available Tags</b><br /><code>{minutes}</code> <code>{seconds}</code><br />',
						'checkout-countdown-for-woocommerce'
					),
					'placeholder' => __( 'We can only hold your item for {minutes} minutes and {seconds} seconds!</p>', 'checkout-countdown-for-woocommerce' ),
					'type'        => 'wysiwyg',
				...
				// Adds filter PREFIX_extend_fields
				$settings_fields = $this->extendable( $settings_fields, 'extend_setting_fields' );
		...
		function get_settings_sidebar() {
			...
			// Adds filter PREFIX_extend_sidebar
			$sidebar = $this->extendable( $sidebar, 'extend_settings_sidebar' );
		 ...
		 * Extendable
		 */
		protected function extendable( $default_array, $filter_name, $extend = true ) {
			...
			$extended = array();
			$extended = apply_filters( $this->setup['prefix'] . '_' . $filter_name, $extended );

			if ( is_array( $extended ) ) {
					return array_merge( $default_array, $extended );
			} else {
