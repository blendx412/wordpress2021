...
class WpWoof_Feed_Manage_list extends Wpwoof_Feed_List_Table {
    ...
    function column_default($item, $column_name) {
        ...
        switch ($column_name) {
            ...
            case 'feedname':
                $addStr ="";
                if(!empty($itemInfo['status_feed']) && $itemInfo['status_feed']!='finished' ){ /**/
                    if( $itemInfo['status_feed']=='starting') {
                        $addStr ="In progress";
                        $addStr='<img id="spinner'.$option_id.'" src="'.WPWOOF_ASSETS_URL.'img/spinner-2x.gif" style="width:16px;" title="'.htmlspecialchars($addStr,ENT_QUOTES).'" alt="'.htmlspecialchars($addStr,ENT_QUOTES).'" />&nbsp;';
                    }  else {
                ...
                return  $addStr.$itemInfo['feed_name'];
            case 'feed_type':
            ...
            case 'feedtaxcountry':
                $answ = "";                             
                if(isset($itemInfo['field_mapping']['tax']['value'])){
            ...
            case 'feedcategory':
                return !empty($itemInfo['feed_category_all']) ? 'All' :  $this->get_category_name((empty($itemInfo['feed_category']) ? '' : $itemInfo['feed_category']));
            
            case 'feeddate':
                return date('d/m/Y H:i:s', $itemInfo['added_time']);
            ...
            case 'feedaction':
                ...
                $edit   = add_query_arg( array('tab'=>1,'edit'=>$option_id), $menu_url);
                $delete = add_query_arg( array('tab'=>0,'delete'=>$option_id), $menu_url);
                ...
                $return  = "<a disabled='disabled' target='_blank' class='button' href='$view'>" . ($itemInfo['feed_type']=="adsensecustom"  ? __('CSV Direct Link', 'woocommerce_wpwoof') : __('Open') ) . "</a>&nbsp;";
                $return .= "<a disabled='disabled' class='button' href='$edit'>" . __('Edit') . '</a>';
                $return .= "&nbsp;<a disabled='disabled' class='button' href='$delete'>" . __('Delete') . '</a>';
                
                return $return;

            case 'feedupdate':

                $update = add_query_arg( array('tab'=>0,'update'=>$option_id), $menu_url);
                $return = "&nbsp;<a disabled='disabled' id='wpwoof_status_".$option_id."a' class='button regenerate' href='$update'>" . __('Regenerate') . "</a>"."<div class='wpwoof_statusbar' id='wpwoof_status_".$option_id."' data-feedid='".$option_id."'><img id='wpwoof_img_".$option_id."' style='margin-left: -2px;' src='".WPWOOF_URL."/assets/img/bar.png' /></div>";
            ...
            case 'feeddownload':
                ...
                $return = ($itemInfo['feed_type']=="adsensecustom" ) ? '' : '<a disabled="disabled" id="wpwoof_status_'.$option_id.'x" class="button button-secondary" href="'.$download_xml.'" target="_blank">XML</a>';
                $return .= '<a disabled="disabled" id="wpwoof_status_'.$option_id.'c" class="button button-secondary regenerate" href="'.$download_csv.'" target="_blank">CSV</a>';
