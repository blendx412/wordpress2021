...
class HM_Product_Sales_Report_Pro {
	...
	/*
		...
		Modified by Jonathan Hall, Daniel von Mitschke, and/or others 2019-12-17 and earlier
	*/
		...
		foreach ($_POST['fields'] as $field) {
			if (substr($field, 0, 18) == 'order_item_total::') {
				...
				if (in_array($_POST['groupby'], array('o_builtin::order_month', 'o_builtin::order_quarter', 'o_builtin::order_year', 'o_builtin::order_date', 'o_builtin::order_day'))) {
					...
					$fieldName = str_replace(array(' ', '-'), '', esc_sql(substr($_POST['groupby'], 2))); // Remove spaces for security
					$dataParams[$fieldName] = array(
	...
	/*
		The following function contains code copied from from WooCommerce; see license/woocommerce-license.txt for copyright and licensing information
		Modified by Jonathan Hall and/or others 2019-09-10 and earlier
	*/
			...
			if ($field == 'builtin::groupby_field') {
				...
				if (!empty($_POST['groupby']) && $_POST['groupby'] != 'i_builtin::item_price') {
					if (in_array($_POST['groupby'], array('o_builtin::order_month', 'o_builtin::order_quarter', 'o_builtin::order_year', 'o_builtin::order_date', 'o_builtin::order_day'))) {
						...
						$dataParams[substr($_POST['groupby'], 2)] = array(
							'type' => ($_POST['groupby'][0] == 'i' ? 'order_item_meta' : 'meta'),
							'order_item_type' => 'shipping',
							'function' => '',
							'join_type' => 'LEFT',
	...
	*/
	...
	public static function pluginCreditBox() {
		echo('  <h3>About</h3><div id="hm_psr_about">
					<div style="background-color: #ff6600; max-width: 500px; text-align: center; padding: 20px; color: #fff;">
						<a href="http://potentplugins.com/?utm_source=product-sales-report-pro&amp;utm_medium=link&amp;utm_campaign=wp-plugin-credit-link" target="_blank"><img src="'.esc_url(plugins_url('images/potent-logo.png', __FILE__)).'" alt="Potent Plugins" style="max-width: 100%;" /></a><br>
						by <a href="https://aspengrovestudios.com/aspen-grove-studios-acquires-wordpress-and-woocommerce-plugin-agency-potent-plugins/?utm_source=product-sales-report-pro&amp;utm_medium=link&amp;utm_campaign=wp-plugin-credit-ags-link" target="_blank">Aspen Grove Studios</a>
					</div>
					<p><strong>Product Sales Report Pro</strong></p>
					<form action="" method="post">
						<strong>License Key:</strong> '.get_option( 'hm_psr_license_key' ));
						wp_nonce_field( 'hm_psr_license_deactivate_nonce', 'hm_psr_license_deactivate_nonce' );
						echo('<button type="submit" name="hm_psr_license_deactivate" value="1" class="button-secondary" style="margin-left: 10px; vertical-align: middle;">Deactivate License</button>
					</form>
				</div>');
	}
}
