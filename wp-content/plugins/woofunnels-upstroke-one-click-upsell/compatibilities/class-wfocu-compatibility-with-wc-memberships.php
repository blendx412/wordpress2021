	...
	public function __construct() {
		if ( $this->is_enable() ) {
			...
			add_action( 'wfocu_front_create_new_order_on_success', function () {
				...
				remove_action( 'woocommerce_order_status_completed', [ $plans, 'grant_access_to_membership_from_order' ], 11 );
			}, 1 );
			add_action( 'wfocu_front_create_new_order_on_success', function () {
				$plans = wc_memberships()->get_plans_instance();
				add_action( 'woocommerce_order_status_completed', [ $plans, 'grant_access_to_membership_from_order' ], 11 );
			}, 20 );
