...
class WFOCU_Compatibility_With_Product_Bundles {
	...
	public function setup_bundle_discount_prices( $product_details, $output, $offer_data, $is_front, $hash_key ) {
		if ( $product_details->data->is_type( 'bundle' ) ) {


			$product_details->regular_price_incl_tax = wc_get_price_including_tax( $product_details->data, array( 'price' => $product_details->data->get_bundle_price() ) ) * $offer_data->fields->{$hash_key}->quantity;
			$product_details->regular_price_excl_tax = wc_get_price_excluding_tax( $product_details->data, array( 'price' => $product_details->data->get_bundle_price() ) ) * $offer_data->fields->{$hash_key}->quantity;
			...
			if ( is_a( $get_product, 'WC_Product' ) && $get_product->is_type( 'bundle' ) ) {
				...
				foreach ( $bundled_items as $item_id => $itemdata ) {
					...
					$configuration[ $item_id ] = array(
						'discount' => 0,
						'args'     => array(
							'total'    => WFOCU_Common::apply_discount( wc_get_price_including_tax( $itemdata->product, [ 'price' => $itemdata->product->get_sale_price() ] ), $current_offer_data->fields->{$product['hash']}, $product ),
							'subtotal' => WFOCU_Common::apply_discount( wc_get_price_including_tax( $itemdata->product, [ 'price' => $itemdata->product->get_sale_price() ] ), $current_offer_data->fields->{$product['hash']}, $product )
						)
					);
