...
class WFOCU_DB_Track extends WFOCU_DB_Base {
	...
	protected function form_ids_for_events( $event_ids ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT `form_id` FROM {$table} WHERE `ID` IN( '%s' )", implode( ',', $event_ids ) ), ARRAY_A ); //db call ok; no-cache ok; WPCS: unprepared SQL ok.
	}
	...
	public function delete( $sess_id ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM %s WHERE event_id like IN( '" . implode( "','", $all_event_ids ) . "' )", $this->get_table_name( true ) ) ); //db call ok; no-cache ok; WPCS: unprepared SQL ok.
