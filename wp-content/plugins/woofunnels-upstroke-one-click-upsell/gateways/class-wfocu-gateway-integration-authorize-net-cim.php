...
class WFOCU_Gateway_Integration_Authorize_Net_CIM extends WFOCU_Gateway {
		...
		if ( $order instanceof WC_Order && $this->key === $order->get_payment_method() ) {
			...
			if ( $this->has_token( $order ) && ! is_checkout_pay_page() ) {
				...
				if ( empty( $order->customer_id ) && ! empty( $customer_id_from_session ) ) {
					$order->customer_id = WFOCU_Core()->data->get( 'authorize_net_cim_customer_id', '', 'gateway' );
				}
		...
		try {
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( isset( $result['messages'] ) && isset( $result['messages']['resultCode'] ) && 'Error' === $result['messages']['resultCode'] ) {
					...
					$order_note = sprintf( __( 'Authorize.net CIM Transaction Failed (%s)', 'woofunnels-upstroke-one-click-upsell' ), $result['messages']['message']['text'] );
		...
		if ( isset( $package['products'] ) && is_array( $package['products'] ) && count( $package['products'] ) > 0 ) {
			foreach ( $package['products'] as $product_data ) {
				...
				$line_items[] = array(
					...
					'name'        => $this->str_to_sane_utf8( $this->str_truncate( $product_data['data']->get_name(), 31 ) ),
					'description' => $this->str_to_sane_utf8( $this->str_truncate( $product_data['data']->get_description(), 255 ) ),
	...
	public function wfocu_modify_refund_request_data( $request_data, $order, $gateway ) {
		...
		if ( isset( $_POST['action'] ) && 'wfocu_admin_refund_offer' === $_POST['action'] ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
