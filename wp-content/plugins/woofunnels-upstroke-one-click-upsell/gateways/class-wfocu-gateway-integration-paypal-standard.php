...
class WFOCU_Gateway_Integration_PayPal_Standard extends WFOCU_Gateway {
	...
	public function maybe_check_ref_transaction() {
		...
		if ( isset( $_GET['wfocu_paypal_check'] ) && current_user_can( 'manage_woocommerce' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		...
		if ( isset( $_GET['wfocu_paypal_check_call'] ) && current_user_can( 'manage_woocommerce' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$response = $this->set_express_checkout( array(
				'currency'     => 'usd',
				'order'        => wc_get_order( wc_clean( $_GET['wfocu_paypal_check_call'] ) ), // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
				'billing_type' => '',
				'return_url'   => $this->get_callback_url( 'wfocu_paypal_create_billing_agreement' ),
				'cancel_url'   => $this->get_callback_url( 'wfocu_paypal_create_billing_agreement' ),
				'notify_url'   => $this->get_callback_url( 'wfocu_paypal_create_billing_agreement' ),
	...
	public function modify_paypal_args( $args, $order ) {
		...
		if ( true === $this->is_enabled() && false === $this->is_reference_trans_enabled() ) {
			...
			add_action( 'wfocu_schedule_email_data_stored', [ $this, 'maybe_save_pending_emails_meta_to_prevent_mails' ], 10, 2 );
			/**
	...
	public function maybe_create_billing() {
		if ( ! isset( $_GET['action'] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return;
		}

		switch ( $_GET['action'] ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			...
			case 'wfocu_paypal_create_billing_agreement':
				// bail if no token
				if ( ! isset( $_GET['token'] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
					return;
				}

				// get token to retrieve checkout details with
				$token = wc_clean( $_GET['token'] );  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public function create_express_checkout_token() {
		...
		$posted_data = WFOCU_Core()->process_offer->parse_posted_data( $_POST ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		...
		try {
			switch ( $_GET['action'] ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
				...
				case 'wfocu_paypal_return':
					...
					if ( isset( $_GET['token'] ) && ! empty( $_GET['token'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification

						$express_checkout_details_response = $this->get_express_checkout_details( wc_clean( $_GET['token'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public function wfocu_woocommerce_paypal_refund_request_data( $request, $order, $amount ) {
		...
		if ( isset( $_POST['txn_id'] ) && ! empty( $_POST['txn_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$request['TRANSACTIONID'] = wc_clean( $_POST['txn_id'] ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
	...
	public function maybe_save_pending_emails_meta_to_prevent_mails( $order, $args ) {
		/**$args = array(
