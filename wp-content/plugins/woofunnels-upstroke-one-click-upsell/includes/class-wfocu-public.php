...
class  WFOCU_Public {
	...
	public function __construct() {
		...
		/**
		 * Hooks for finding the active and running funnels, keeping the products/indexed rules in mind
		 */
		add_filter( 'woocommerce_update_cart_action_cart_updated', array( $this, 'maybe_setup_funnels_on_cart_update' ), 99, 1 );
		add_action( 'woocommerce_add_to_cart', array( $this, 'maybe_setup_funnels' ), 99 );
		add_action( 'woocommerce_cart_item_removed', array( $this, 'maybe_setup_funnels' ), 99 );
		add_action( 'woocommerce_cart_item_restored', array( $this, 'maybe_setup_funnels' ), 99 );
	...
	public function maybe_decide_funnel_on_fragments( $fragments = array() ) {
		...
		if ( ! isset( $_POST['post_data'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return $fragments;
		}
		wp_parse_str( $_POST['post_data'], $arr );    // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	 ...
	 * @param $order_id
	 */
	public function maybe_decide_funnel_on_order( $order_id, $posted_data ) {
		WFOCU_Core()->rules->set_environment_var( 'order', $order_id );
	...
	public function maybe_redirect_to_upsell( $url, $order ) {
		...
		if ( WFOCU_Core()->data->is_funnel_exists() && $get_integration instanceof WFOCU_Gateway && $get_integration->is_enabled( $order ) && ( $get_integration->has_token( $order ) || $get_integration->is_run_without_token() ) ) {
			...
			do_action( 'wfocu_funnel_init_event', WFOCU_Core()->data->get_funnel_id(), WFOCU_WC_Compatibility::get_order_id( $order ), WFOCU_Core()->data->get( 'useremail' ), $order->get_payment_method() );
	...
	public function get_the_upsell_url( $offer ) {
		...
		), WFOCU_Core()->offers->get_the_link( $offer ) );
	}
	...
	public function maybe_setup_upsell_on_cod_or_bacs( $order_id, $posted_data = array() ) {
		...
		if ( WC()->cart->needs_payment() && is_array( $posted_data ) && isset( $posted_data['payment_method'] ) && in_array( $posted_data['payment_method'], array( 'cheque', 'bacs' ), true ) ) {
	...
	public function handle_failed_validation_for_the_offer() {
		...
		if ( isset( $_GET['wfocu-key'] ) && ! empty( $_GET['wfocu-key'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$maybe_get_order = get_posts( array( //phpcs:ignore WordPressVIPMinimum.Functions.RestrictedFunctions.get_posts_get_posts
				...
				'meta_value'  => wc_clean( $_GET['wfocu-key'] ), // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification, WordPress.DB.SlowDBQuery.slow_db_query_meta_value
				'fields'      => 'ids',
			) );
	...
	/**
	 * @hooked over `woocommerce_add_to_cart` || `woocommerce_cart_item_removed` || `woocommerce_cart_item_restored`
	 * On every WooCommerce Add to cart operation perform the funnel setup as force to further find prepare the funnels
	 */
	public function maybe_setup_funnels() {
		WFOCU_Core()->funnels->setup_funnels( true );
	}

	/**
	 * @hooked over `woocommerce_add_to_cart`
	 * On every WooCommerce Add to cart operation perform the funnel setup as force to further find prepare the funnels
	 */
	public function maybe_setup_funnels_on_cart_update( $cart_updated ) {
		WFOCU_Core()->funnels->setup_funnels( true );
	...
	public function if_is_preview() {
		if ( 'loaded' === filter_input( INPUT_GET, 'wfocu_customize', FILTER_SANITIZE_STRING ) || ( isset( $_POST['action'] ) && $_POST['action'] === 'customize_save' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public function maybe_log_thankyou_visited( $order_id ) {
		...
		if ( isset( $_GET['_wfocu_process'] ) && 'no' === $_GET['_wfocu_process'] ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			WFOCU_Core()->log->log( 'Order #' . $order_id . ': Error confirmation logged during upsell process--' . print_r( isset( $_GET['ec'] ) ? wc_clean( $_GET['ec'] ) : '', true ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification, WordPress.PHP.DevelopmentFunctions.error_log_print_r
