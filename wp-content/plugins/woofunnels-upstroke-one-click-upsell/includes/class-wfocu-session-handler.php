...
class WFOCU_Session_Handler {
	...
	public function load_transient_from_cookie() {
		$cookie_value = '';
		if ( isset( $_GET['wfocu-si'] ) ) {   // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$cookie_value = wc_clean( $_GET['wfocu-si'] );  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification

		} elseif ( WFOCU_AJAX_Controller::is_wfocu_front_ajax() ) {
			$cookie_value = wc_clean( $_POST['wfocu-si'] );  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
