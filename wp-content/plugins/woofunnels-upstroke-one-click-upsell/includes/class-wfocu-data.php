...
class WFOCU_Data extends WFOCU_Session_Handler {
	...
	public function load_order( $order_id = 0 ) {
		...
		if ( $order_id === 0 ) {
			$order_id = ( isset( $_GET['order_id'] ) && ( $_GET['order_id'] !== '' ) ) ? wc_clean( $_GET['order_id'] ) : 0;  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
		if ( $order_id !== 0 ) {
	...
	public function set_page( $id = null ) {
		...
		if ( is_numeric( $id ) && $id !== 0 ) {
			$post = get_post( $id ); //phpcs:ignore WordPress.WP.GlobalVariablesOverride.OverrideProhibited
		}
