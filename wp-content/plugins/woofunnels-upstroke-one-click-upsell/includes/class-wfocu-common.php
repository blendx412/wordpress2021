...
class WFOCU_Common {
	...
	public static function wfocu_rule_json_search_coupons() {
		...
		foreach ( $ids as $id ) {
			$found_coupons[ $id ] = sprintf( /* translators: $1: coupon title */ esc_html__( '%1$s', 'woocommerce' ), get_the_title( $id ) );
		}
	...
	public static function register_post_type() {
		...
		$rewrite_slug = apply_filters( 'wfocu_offer_post_type_slug', WFOCU_Core()->data->get_option( 'offer_post_type_slug' ) );
	...
	public static function ajax_render_rule_choice( $options ) {
		...
		if ( defined( 'DOING_AJAX' ) && true === DOING_AJAX && isset( $_POST['action'] ) && $_POST['action'] === 'wfocu_change_rule_type' ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$is_ajax = true;
		}
		if ( $is_ajax ) {
	...
	public static function render_rule_choice_template( $options ) {
		...
		$operator_args = array(
			'input'   => 'select',
			'name'    => 'wfocu_rule[' . $options['category'] . '][<%= groupId %>][<%= ruleId %>][operator]',
			/* phpcs:ignore WordPressVIPMinimum.Security.Underscorejs.OutputNotation,WordPress.Security.EscapeOutput.OutputNotEscaped */
			'choices' => $operators,
		);
		echo '<td class="operator">';
		if ( ! empty( $operators ) ) {
	...
	public static function search_products( $term, $include_variations = false ) {
		...
		$product_ids = $wpdb->get_col( $wpdb->prepare( "SELECT DISTINCT posts.ID FROM {$wpdb->posts} posts
				...
				ORDER BY posts.post_parent ASC, posts.post_title ASC", $like_term, $like_term ) ); //db call ok; no-cache ok; WPCS: unprepared SQL ok.
