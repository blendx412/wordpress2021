...
if ( ! function_exists( 'wc_wfocu_dropdown_variation_attribute_options' ) ) {
	...
	function wc_wfocu_dropdown_variation_attribute_options( $args = array() ) {
			...
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $term ) {
					if ( in_array( $term->slug, $options, true ) ) {
						...
						$html .= '<option value="' . esc_attr( WFOCU_Common::handle_single_quote_variation( $term->slug ) ) . '" ' . selected( sanitize_title( $args['selected'] ), $term->slug, false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $term->name ) ) . '</option>';
		...
		echo apply_filters_deprecated( 'woocommerce_wfocu_dropdown_variation_attribute_options_html', [ $html ], '2.0', 'This filter is removed as our javascript does not support any other HTML structure.' ); // WPCS: XSS ok.
	}
}
