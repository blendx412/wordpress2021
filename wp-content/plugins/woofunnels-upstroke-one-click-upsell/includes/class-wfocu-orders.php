...
class WFOCU_Orders {
	...
	public function clean_comments() {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->comments WHERE `comment_post_ID` = %d", 0 ) );   //db call ok; no-cache ok; WPCS: unprepared SQL ok.
	...
	public function maybe_show_order_details( $atts ) {
		...
		if ( ! empty( $_GET['key'] ) || ! empty( $_GET['ctp_order_key'] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$order_key = '';

			if ( ! empty( $_GET['key'] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
				$order_key = wc_clean( $_GET['key'] );  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			}
			 ...
			 */
			if ( ! empty( $_GET['ctp_order_key'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
				$order_key = wc_clean( $_GET['ctp_order_key'] ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			}
