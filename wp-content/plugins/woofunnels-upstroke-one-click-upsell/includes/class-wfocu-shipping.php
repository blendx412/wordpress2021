...
class WFOCU_Shipping {
	...
	public function __construct() {
		...
		add_action( 'wfocu_funnel_decided', array( $this, 'maybe_save_shipping_id' ) );
		...
		if ( 'flat' === $output->shipping_preferece ) {
			...
			if ( 0 < intval( $product_details->shipping_cost_flat ) ) {
				$product_details->shipping = array(
					...
					'shipping'      => array(
						'fixed' => array(
							'cost'         => $product_details->shipping_cost_flat,
							'shipping_tax' => $this->get_flat_shipping_rates( $product_details->shipping_cost_flat ),
	...
	public function maybe_save_shipping_id() {
		$chosen_shipping_methods = WFOCU_Core()->data->get_posted( 'shipping_method' );
		WFOCU_Core()->data->set( 'chosen_shipping_methods', $chosen_shipping_methods );
