...
class WFOCU_Customizer {
	...
	public function __construct() {
		if ( isset( $_REQUEST['offer_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$this->offer_id = wc_clean( $_REQUEST['offer_id'] ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
	...
	public function maybe_load_customizer() {
		if ( isset( $_REQUEST['wfocu_customize'] ) && $_REQUEST['wfocu_customize'] === 'loaded' && isset( $_REQUEST['offer_id'] ) && $_REQUEST['offer_id'] > 0 ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
...
.ast-control-tooltip {display: none !important;}</style>';
				...
				if ( $this->template !== '' ) {
					...
					if ( is_array( $locate_template ) && file_exists( $locate_template['path'] ) ) {
						...
						if ( method_exists( $this->template_ins, 'get_slug' ) ) {
							...
							if ( isset( $_REQUEST['customized'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
								$change_set = json_decode( wc_clean( $_REQUEST['customized'] ), true ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public function is_valid_customizer_template( $selected = 'sp-classic' ) {
		 ...
		 */
		if ( 'mp-grid' === $selected || 'mp-list' === $selected ) {
			return true;
		}
	...
	public function customizer_product_check() {
		...
		if ( WFOCU_Core()->template_loader->is_customizer_preview() && WFOCU_Core()->template_loader->is_valid_state_for_data_setup() ) {
	...
	public function setup_offer_for_wfocukirki() {
		...
		add_action( 'wfocu_header_print_in_head', array( WFOCU_Core()->template_loader, 'print_internal_css' ) );
