...
class WFOCU_AJAX_Controller {
	...
	public static function handle_charge() {
		...
		try {
			...
			$response = false;
			$message  = __( 'Unable to process, Caught PHP Error!', 'woofunnels-upstroke-one-click-upsell' );
			WFOCU_Core()->log->log( 'Error Captured: ' . print_r( $e->getMessage() . " <-- Generated on" . $e->getFile() . ":" . $e->getLine(), true ) ); // @codingStandardsIgnoreLine

			/**
			 * If error captured during charge process, then handle as failture
			 */
			if ( 0 === did_action( 'wfocu_offer_charge_complete' ) ) {
				$data = WFOCU_Core()->process_offer->_handle_upsell_charge( $response );
			} else {
				$data['redirect_url'] = WFOCU_Core()->public->get_clean_order_received_url();
			}
		} catch ( WFOCU_Payment_Gateway_Exception $e ) {
			$get_order->add_order_note( sprintf( esc_attr__( 'Offer payment failed, Reason: %s', 'woofunnels-upstroke-one-click-upsell' ), esc_attr( $e->getMessage() ) ) );

			WFOCU_Core()->log->log( 'Order #' . WFOCU_WC_Compatibility::get_order_id( $get_order ) . ': Payment Failed' );
			WFOCU_Core()->log->log( print_r( $e->getMessage(), true ) );  // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r

			$message = __( 'Unable to process, Caught WFOCU_Payment_Gateway_Exception Exception!', 'woofunnels-upstroke-one-click-upsell' );
			/**
			 * If error captured from gateway itself, than handle as failure
			 */
			$data = WFOCU_Core()->process_offer->_handle_upsell_charge( $response );

		} catch ( Exception $e ) {
			$response = false;
			$message  = __( 'Unable to process, Caught PHP Exception!', 'woofunnels-upstroke-one-click-upsell' );
			WFOCU_Core()->log->log( 'Exception Captured: Details' . print_r( $e->getMessage(), true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
	...
	public static function update_template() {
		...
		if ( is_object( $meta ) ) {
			...
			do_action( 'wfocu_template_selected', wc_clean( $_POST['template_group'] ), wc_clean( $_POST['template'] ), $offer, $meta );
		}
		if ( '' !== $funnel_id ) {
			WFOCU_Common::update_funnel_time( $funnel_id );
		}
			...
			if ( ! empty( $data_funnels['steps'] ) ) {
				foreach ( $data_funnels['steps'] as $steps ) {
					...
					if ( ! is_wp_error( $offer_id_new ) && $offer_id_new ) {
						...
						$post_meta_all = $wpdb->get_results( "SELECT meta_key, meta_value FROM $wpdb->postmeta WHERE post_id=$get_offer" ); //db call ok; no-cache ok; WPCS: unprepared SQL ok.
