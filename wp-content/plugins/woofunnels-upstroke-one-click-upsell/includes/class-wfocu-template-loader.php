...
class WFOCU_Template_loader {
	...
	public function __construct() {
		...
		add_action( 'template_redirect', function () {
			add_filter( 'template_include', array( $this, 'maybe_load' ), 99 ); //phpcs:ignore WordPressVIPMinimum.Variables.VariableAnalysis.UndefinedVariable
		}, 999 );

		add_action( 'wfocu_header_print_in_head', array( $this, 'typography_custom_css' ) );

		add_filter( 'theme_' . WFOCU_Common::get_offer_post_type_slug() . '_templates', function () {
			return [];
		}, 999 );
	...
	public function is_customizer_preview() {
		if ( isset( $_REQUEST['wfocu_customize'] ) && 'loaded' === $_REQUEST['wfocu_customize'] ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return true;
		}
