...
class WFOCU_Wizard {
	...
	public static function setup_wizard() {
		...
		if ( empty( $_GET['page'] ) || 'woofunnels' !== $_GET['page'] ) {   // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return;
		}
		if ( empty( $_GET['tab'] ) || WFOCU_SLUG . '-wizard' !== $_GET['tab'] ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return;
		}

		ob_end_clean();

		self::$step = isset( $_GET['step'] ) ? sanitize_key( $_GET['step'] ) : current( array_keys( self::$steps ) );  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public static function setup_wizard_header() {
		...
		<h1 id="wc-logo"><img width="200px;" src="//storage.googleapis.com/upstroke/woofunnel_logo.png"/>
			<?php
			//			$image_url = do_shortcode(get_theme_mod( 'site_logo', get_template_directory_uri().'/assets/img/logo.png'));
			//			$image_url = apply_filters('wfocu_setup_logo_image',$image_url);
			//			if ( $image_url ) {
	...
	public static function wfocu_setup_introduction() {
		?>
		<h1><?php _e( 'Thank you for choosing UpStroke from WooFunnels.', WFOCU_SLUG ); ?></h1>
	...
	public static function wfocu_setup_ready() {
		...
		<p><?php printf( __( 'You are ready to setup your first funnel.', WFOCU_SLUG ) ); ?></p>
