...
class WFOCU_Admin {
	...
	public function register_admin_menu() {
		...
		add_submenu_page( 'woofunnels', __( 'UpStroke', 'woofunnels-upstroke-one-click-upsell' ), __( 'UpStroke', 'woofunnels-upstroke-one-click-upsell' ), 'manage_woocommerce', 'upstroke', array(
			$this,
			'upstroke_page',
		) );
	}
	...
	public function admin_enqueue_assets() {
		...
		if ( $this->is_upstroke_page() ) {
			WFOCU_Core()->funnels->setup_funnel_options( ( isset( $_GET['edit'] ) ? wc_clean( $_GET['edit'] ) : 0 ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
		...
		if ( WFOCU_Common::is_load_admin_assets( 'all' ) ) {
			...
			wp_enqueue_script( 'wfocu-admin-builder', $this->admin_url . '/assets/js/wfocu-admin-builder.js', array( 'jquery', 'wfocu-swal', 'wfocu-vuejs', 'wfocu-vue-vfg' ), WFOCU_VERSION_DEV );
		...
		$data                = array(
			...
			'alerts'                                => array(
				...
				'import_template'      => array(
					...
					'text'              => __( 'It will take around 5-10 seconds to import this template in your store.', 'woofunnels-upstroke-one-click-upsell' ),
			...
			'forms_labels'                          => array(
				...
				'settings'              => array(
					...
					'funnel_priority'           => array(
						...
						'hint'  => __( "There may be a chance that Rules can be set up in a way that Two Upsell Funnels can trigger.\n In such cases, Funnel Priority is used to determine which Funnel will trigger. Priority Number 1 is considered highest.", 'woofunnels-upstroke-one-click-upsell' ),
				...
				'global_settings' => array(
					...
					'gad_key'                       => array(
						'label' => __( 'Conversion ID', 'woofunnels-upstroke-one-click-upsell' ),
					...
					'gad_conversion_label'          => array(
						'label' => __( 'Conversion Label', 'woofunnels-upstroke-one-click-upsell' ),
						'hint'  => __( '<a target="_blank" href="https://buildwoofunnels.com/docs/upstroke/global-settings/tracking-analytics/#google-ads-tracking">Learn more about how to get the conversion label</a>', 'woofunnels-upstroke-one-click-upsell' ),
 ...
 <br/><br/> <strong>Why it is needed?</strong> <br/>There can be additional processes such as sending of data to external CRMs which can trigger when order is successful. By having an intermediate order state, we wait for upsell funnel to get complete. Once it is done order status automatically moves to successful order state. This method enables additional processes to process correct order data.', 'woofunnels-upstroke-one-click-upsell' ),
					...
					'create_new_order_status_fail' => array(
						...
						'hint'   => __( '<strong> Why it is needed?</strong> <br/>
...
In such scenarios a separate order is created for your record and is created and marked as failed. ', 'woofunnels-upstroke-one-click-upsell' ),
					...
					'ttl_funnel'                   => array(
						...
						'hint'  => __( '<strong> Why it is needed? </strong><br/>Sometimes users may keep Offer Page open and not take a decision. Set up a realistic time in minutes after which funnel forcefully ends.
This setting will determine time of Order Confirmation emails if it set to "When Funnel Ends". <br/> If you are not sure keep it by default to 15 mins', 'woofunnels-upstroke-one-click-upsell' ),
				...
				'offer_settings'                     => array(
					...
					'terminate_if_accepted'        => array(
						'label' => __( 'If this offer is accepted. Customer will be redirected to thank you page.', 'woofunnels-upstroke-one-click-upsell' ),
					),
					'terminate_if_declined'        => array(
						'label' => __( 'If this offer is declined. Customer will be redirected to thank you page.', 'woofunnels-upstroke-one-click-upsell' ),
					),
					...
					'qty_selector'                 => array(
						'label' => __( 'Allow customer to chose the quantity while purchasing this upsell product(s)', 'woofunnels-upstroke-one-click-upsell' ),
					),
					'qty_max_label'                => array(
	...
	public function upstroke_page() {
		if ( isset( $_GET['page'] ) && 'upstroke' === $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			...
			} elseif ( isset( $_GET['tab'] ) && $_GET['tab'] === 'settings' ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public function js_variables() {
		...
		$data['button_texts'] = array(
			'importingtext' => __( 'Please wait', 'woofunnels-upstroke-one-click-upsell' ),
		...
		$required_states             = array( 0, 1, 2, 3, 6 );
		$state                       = absint( WooFunnels_Dashboard::$classes['WooFunnels_DB_Updater']->get_upgrade_state() );
		$data['bwf_needs_indexning'] = in_array( $state, $required_states, true );
		...
		$data['preset_texts']        = array(
			'success' => __( 'Preset applied to ', 'woofunnels-upstroke-one-click-upsell' ),
		);
	...
	public function is_upstroke_page( $section = '' ) {
		if ( isset( $_GET['page'] ) && $_GET['page'] === 'upstroke' && '' === $section ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return true;
		}

		if ( isset( $_GET['page'] ) && $_GET['page'] === 'upstroke' && isset( $_GET['section'] ) && $_GET['section'] === $section ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return true;
		}
	...
	public function maybe_set_funnel_id() {
		...
		if ( $this->is_upstroke_page() && isset( $_GET['edit'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification

			WFOCU_Core()->funnels->set_funnel_id( wc_clean( $_GET['edit'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
	}
	...
	public function maybe_activate_post() {
		...
		if ( isset( $_GET['action'] ) && $_GET['action'] === 'wfocu-post-activate' ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public function maybe_deactivate_post() {
		if ( isset( $_GET['action'] ) && $_GET['action'] === 'wfocu-post-deactivate' ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public function maybe_handle_http_referer() {
		if ( $this->is_upstroke_page() && ! empty( $_REQUEST['_wp_http_referer'] ) && ! empty( $_REQUEST['REQUEST_URI'] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
	...
	public function maybe_show_wizard() {
		if ( empty( $_GET['page'] ) || 'upstroke' !== $_GET['page'] ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return;
		}
		if ( isset( $_GET['tab'] ) && strpos( wc_clean( $_GET['tab'] ), 'wizard' ) !== false ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			return;
		}
	...
	public function maybe_remove_all_notices_on_page() {
		if ( isset( $_GET['page'] ) && 'upstroke' === $_GET['page'] && isset( $_GET['section'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
...
<p style="font-size: 12px;"><em> ' . sprintf( esc_html__( 'UpStroke: %s' ), wc_price( $result, array( 'currency' => get_option( 'woocommerce_currency' ) ) ) ) . '</em></p>';
	...
	public function no_conflict_mode_style() {
		...
		$wfocu_required_styles = apply_filters( 'wfocu_no_conflict_styles', array(
			'common'   => array( 'wfocu-funnel-bg', 'wfocu-izimodal', 'wfocu-modal', 'wfocu-admin', 'woocommerce_admin_styles' ),
			'settings' => array(
				'wfocu-modal',
				'wfocu-vue-multiselect',
			),
	...
	public function no_conflict_mode( &$wp_objects, $wp_required_objects, $wfocu_required_objects, $type = 'scripts' ) {
		...
		$page_objects = isset( $wfocu_required_objects[ $section ] ) ? $wfocu_required_objects[ $section ] : false;
		...
		$required_objects = array_merge( $wp_required_objects, $wfocu_required_objects['common'], $page_objects );
