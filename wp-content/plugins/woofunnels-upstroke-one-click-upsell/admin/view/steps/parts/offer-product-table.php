<form class="wfocu_forms_wrap" data-wfoaction="save_funnel_offer_products" v-bind:data-offer_id="current_offer_id">
    ...
    <div v-if="current_offer_id>0 && selected_product>0">
        ...
        <div class="wfocu_product_list_wrap " v-if="Object.keys(products).length>0">
            <h2><?php echo __( 'Products', 'woofunnels-upstroke-one-click-upsell' ); ?></h2>
                    ...
                    <tr v-for="(product, index) in products" v-bind:id="current_offer_id+index" v-bind:data-proid="index" class="product_tb_row">
                        <td>
                            <table class="main_products">
                                ...
                                <tr>
                                    <th class="product_th"><?php _e( "Product", 'woofunnels-upstroke-one-click-upsell' ) ?></th>
                                    <th><?php _e( "Type", 'woofunnels-upstroke-one-click-upsell' ); ?></th>
                                    <th><?php _e( "Discount on Regular Price", 'woofunnels-upstroke-one-click-upsell' ); ?></th>
                                    <th><?php _e( "Quantity", 'woofunnels-upstroke-one-click-upsell' ); ?></th>
                                    <th><?php _e( "Flat Shipping", 'woofunnels-upstroke-one-click-upsell' ); ?></th>
                                    <th><?php _e( " ", 'woofunnels-upstroke-one-click-upsell' ); ?></th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr class="wfocu-product-row">
                                    <td>
                                        ...
                                        <div class="product_details">
                                            ...
                                            <div class="product_options">
                                                <p v-if="typeof product.regular_price!=='undefined'"><?php _e( 'Regular Price', 'woofunnels-upstroke-one-click-upsell' ); ?>:
                                                    <span v-html="product.regular_price"></span>
                                                </p>


                                                <p v-if="typeof product.price!=='undefined'"><?php _e( 'Sale Price', 'woofunnels-upstroke-one-click-upsell' ); ?>: <span v-html="product.price"></span></p>

                                                  <div v-if="product.type !== 'variable' && product.type !== 'variable-subscription'">
                                                <p><?php _e( 'Offer Price', 'woofunnels-upstroke-one-click-upsell' ); ?>: <span v-bind:class="'wfocu_of_price_'+index" v-html="offer_price_html(product)"></span>    </p>    <?php WFOCU_Core()->admin->tooltip( 'Prices are <span v-bind:class="\'wfocu_of_price_data_\'+index" v-html="prepare_price_help_html(product)"></span>'); ?>
                                                  </div>
                                    ...
                                    <td>
                                        <div class="product_type"><p>{{product.type != undefined ? product.type : `simple`}}</p></div>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <select class="product_discount_type" v-model="product.discount_type" v-bind:name="'offers['+current_offer_id+'][products]['+index+'][discount_type]'" class="discount_number" v-on:change="update_offer_price($event,index)">
                                            <option value="percentage"><?php _e( '%', 'woofunnels-upstroke-one-click-upsell' ); ?></option>
                                            <option value="fixed"><?php _e( get_woocommerce_currency_symbol(), 'woofunnels-upstroke-one-click-upsell' ); ?></option>
                                        </select>
                                    </td>
