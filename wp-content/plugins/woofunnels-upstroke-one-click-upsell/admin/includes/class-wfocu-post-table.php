...
class WFOCU_Post_Table extends WP_List_Table {
	...
	public function column_name( $item ) {
		...
		foreach ( $item['row_actions'] as $k => $action ) {
			...
			if ( $k === 'delete' ) {
				$column_string .= '<span class="' . $action['action'] . '"><a href="' . $action['link'] . '" ' . $action['attrs'] . ' onclick="return confirm(\'Are you sure you want to permanently delete this funnel? This action cannot be undone.\')">' . $action['text'] . '</a>';
	...
	public function column_preview( $item ) {
		return sprintf( '<a href="javascript:void(0);" class="wfocu-preview" data-funnel-id="%d" title="Preview">%s</a>', $item['id'], __( 'Preview', 'woofunnels-upstroke-one-click-upsell' ) );
	...
	public function get_columns() {
		$columns = array(
			...
			'preview'     => '&nbsp',
			'description' => __( 'Description', 'woofunnels-upstroke-one-click-upsell' ),
	...
	public function search_box( $text = '', $input_id = 'wfocu' ) {
		...
		if ( ! empty( $_REQUEST['orderby'] ) ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			echo '<input type="hidden" name="orderby" value="' . esc_attr( wc_clean( $_REQUEST['orderby'] ) ) . '" />'; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
		if ( ! empty( $_REQUEST['order'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			echo '<input type="hidden" name="order" value="' . esc_attr( wc_clean( $_REQUEST['order'] ) ) . '" />'; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
		if ( ! empty( $_REQUEST['post_mime_type'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			echo '<input type="hidden" name="post_mime_type" value="' . esc_attr( wc_clean( $_REQUEST['post_mime_type'] ) ) . '" />'; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
		if ( ! empty( $_REQUEST['detached'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			echo '<input type="hidden" name="detached" value="' . esc_attr( wc_clean( $_REQUEST['detached'] ) ) . '" />'; // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
		?>
        <p class="search-box">
	...
	public static function render_trigger_nav() {
		...
		if ( isset( $_GET['status'] ) && '' !== $_GET['status'] ) {  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
			$current_status = wc_clean( $_GET['status'] );  // phpcs:ignore WordPress.Security.NonceVerification.NoNonceVerification
		}
