            ...
            self.design_settings = new Vue(
                {
                    ...
                    methods: {
                        ...
                        update_template: function (template_id) {
                            this.isimporting = "yes";

                            let offer_id = self.get_offer_id();
                            wfocu.offers[offer_id].template = template_id;
                            wfocu.offers[offer_id].template_group = this.template_group;
                            let data = {"id": funnel_id, 'offer_id': offer_id, 'template': template_id, 'template_group': this.template_group, '_nonce': wfocuParams.ajax_nonce_update_template};
                            let wp_ajax = wfo_ajax();
                                    ...
                                    $.extend(
                                        {
                                            ...
                                            cancelButtonColor: '#e33b3b',
                                            confirmButtonText: '',
                                        }, wfocuParams.alerts.import_template
                                    )
                                ).then(
                                    (result) => {
                                        if (result.value) {
                    ...
                    success: function (rsp) {
                        ...
                        if (rsp.status == true) {
                            setTimeout(function () {
                                if ($('#modal-template_success').length > 0) {
                                    $("#modal-template_success").iziModal('open');
                                    $("#modal-template_success").iziModal('setTitle', wfocu.preset_texts.success + ' ' + temp_name);
                                }
                                $('.wfocu_customize_template').css('display', 'none');
                                $('.wfocu_apply_template').css('display', 'inline-block');
