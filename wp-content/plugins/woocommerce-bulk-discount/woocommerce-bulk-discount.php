...
if ( !class_exists( 'Woo_Bulk_Discount_Plugin_t4m' ) ) {
	...
	class Woo_Bulk_Discount_Plugin_t4m {
		...
		protected function get_discounted_coeff( $product_id, $quantity ) {
            ...
            // Added different approach to discount - if is defined discount for single product then take it from there not globally
			$configurer = get_page_by_title( 'wc_bulk_discount_configurer', OBJECT, 'product' );
			if ( $configurer && $configurer->ID && $configurer->post_status == 'private' && (get_field('different_buttons', $product_id) != 'True') ) {
		...
		public function filter_subtotal_price( $price, $values ) {
			...
			$new_css = esc_attr( get_option( 'woocommerce_t4m_css_new_price', 'color: #4AB915; font-weight: bold;' ) );
			$bulk_info = sprintf( __( '%s popust', 'wc_bulk_discount' ), ( get_option( 'woocommerce_t4m_discount_type', '' ) == 'flat' || get_option( 'woocommerce_t4m_discount_type', '' ) == 'fixed' ? get_woocommerce_currency_symbol() . $coeff : ( round( ( 1 - $coeff ) * 100, 2 ) . "%" ) ) );
		...
		public function filter_subtotal_order_price( $price, $values, $order ) {
			...
			$new_css = esc_attr( get_option( 'woocommerce_t4m_css_new_price', 'color: #4AB915; font-weight: bold;' ) );
			$bulk_info = sprintf( __( '%s popust', 'wc_bulk_discount' ), ( $discount_type == 'flat' || $discount_type == 'fixed' ? get_woocommerce_currency_symbol() . $coeff : ( round( ( 1 - $coeff ) * 100, 2 ) . "%" ) ) );
