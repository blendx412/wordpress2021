 ...
 * @subpackage    WC_Report_Upsells_By_Product
 * @category    Class
 */
class WC_Report_Upsells_By_Product extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		if ( count( $this->product_ids ) == 1 && get_post_type( $this->product_ids[0] ) == 'wfocu_funnel' ) {
			$upsell_legend_title   = sprintf( __( '%s Upsells for all the items from this funnel.', 'woofunnels-upstroke-reports' ), '<strong>' . wc_price( $item_upsells ) . '</strong>' );
			$accepted_legend_title = sprintf( __( '%s offer accepted for the products from this funnel', 'woofunnels-upstroke-reports' ), '<strong>' . ( $item_count ) . '</strong>' );
		} else {
			$upsell_legend_title   = sprintf( __( '%s Upsells for the selected items', 'woofunnels-upstroke-reports' ), '<strong>' . wc_price( $item_upsells ) . '</strong>' );
			$accepted_legend_title = sprintf( __( '%s offer accepted for the selected product', 'woofunnels-upstroke-reports' ), '<strong>' . ( $item_count ) . '</strong>' );
		}
	...
	public function products_widget() {
        ...
        <h4 class="section_title"><span><?php esc_html_e( 'Top Upsells', 'woofunnels-upstroke-reports' ); ?></span></h4>
        <div class="section">
            <table cellspacing="0">
				<?php
				...
				if ( count( $top_upsells ) > 0 ) {
					...
					foreach ( $top_upsells as $top_upsell ) {
						echo '<tr class="' . ( in_array( $top_upsell->product_id, $this->product_ids ) ? 'active' : '' ) . '">
							...
							<td class="sparkline">' . $this->upsells_sparkline( $top_upsell->product_id, 7, 'count' ) . '</td>
						</tr>';
					}
				...
				if ( count( $top_earners ) > 0 ) {
					...
					foreach ( $top_earners as $top_earner ) {
						echo '<tr class="' . ( in_array( $top_earner->product_id, $this->product_ids ) ? 'active' : '' ) . '">
							<td class="count">' . wc_price( $top_earner->upsells ) . '</td>
							...
							<td class="sparkline">' . $this->upsells_sparkline( $top_earner->product_id, 7, 'upsells' ) . '</td>
						</tr>';
					}
					// @codingStandardsIgnoreEnd
				} else {
	...
	public function get_export_button() {
        ...
        >
			<?php _e( 'Export CSV', 'woofunnels-upstroke-reports' ); ?>
        </a>
		<?php
	}
	...
	public function upsells_sparkline( $object_id = '', $days = 7, $type = 'upsells' ) {
		...
		if ( 'upsells' === $type ) {
			...
			$tooltip        = sprintf( __( 'Upsells %1$s worth in the last %2$d days', 'woofunnels-upstroke-reports' ), strip_tags( wc_price( $total_upsells ) ), $days );
			...
			$tooltip        = sprintf( _n( 'Accepted 1 product as offer in the last %2$d days', 'Accepted %1$d producs as offers in the last %2$d days', $total_count, 'woofunnels-upstroke-reports' ), $total_count, $days );
	...
	public function get_main_chart() {
		...
		if ( empty( $this->product_ids ) ) {
            ...
            <div class="chart-container">
                <p class="chart-prompt"><?php esc_html_e( 'Choose a product/funnel to view stats', 'woofunnels-upstroke-reports' ); ?></p>
            </div>
			<?php
                ...
                jQuery(function () {
                    ...
                    var drawGraph = function (highlight) {
                        ...
                        var series = [
                            ...
                            {
                                label: "<?php echo esc_js( __( 'Upsells amount', 'woofunnels-upstroke-reports' ) ) ?>",
                                data: upsells_data.product_accepted_upsells,
                                yaxis: 2,
                                color: '<?php echo $this->chart_colours['upsells_amount']; ?>',
