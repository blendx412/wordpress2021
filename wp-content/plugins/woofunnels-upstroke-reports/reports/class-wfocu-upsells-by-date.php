...
class WC_Report_Upsells_By_Date extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		$this->report_data->grass_upsells           = array_sum( wp_list_pluck( $this->report_data->upsells, 'total_upsells' ) );
		$this->report_data->average_grass_upsells   = $this->report_data->grass_upsells / ( $this->chart_interval + 1 );
	...
	public function get_chart_legend() {
		...
		switch ( $this->chart_groupby ) {
			case 'day':
				/* translators: %s: average gross daily upsells */ $average_upsell_title = sprintf( __( '%s average gross daily upsells', 'woofunnels-upstroke-reports' ), '<strong>' . wc_price( $data->average_grass_upsells ) . '</strong>' );
				break;
			case 'month':
			default:
				/* translators: %s: average gross monthly upsells */ $average_upsell_title = sprintf( __( '%s average gross monthly upsells', 'woofunnels-upstroke-reports' ), '<strong>' . wc_price( $data->average_grass_upsells ) . '</strong>' );
				break;
		}

		$legend[] = array(
			'title'            => sprintf( __( '%s Gross Upsells in this period', 'woofunnels-upstroke-reports' ), '<strong>' . wc_price( $data->grass_upsells ) . '</strong>' ),
			'placeholder'      => __( 'The sum of revenue genereted through upsells product.', 'woofunnels-upstroke-reports' ),
			'color'            => $this->chart_colours['grass_upsells'],
			'highlight_series' => 3,
		);

		$legend[] = array(
			'title'            => $average_upsell_title,
			'placeholder'      => __( 'Average revenue genereted though upsells product.', 'woofunnels-upstroke-reports' ),
			'color'            => $this->chart_colours['average_grass_upsells'],
			'highlight_series' => 4,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s Total number of times funnels are triggered in this period', 'woofunnels-upstroke-reports' ), '<strong>' . $data->total_funnels_triggered . '</strong>' ),
			'placeholder'      => __( 'The number of funnels initiated.', 'woofunnels-upstroke-reports' ),
			'color'            => $this->chart_colours['funnels_triggered'],
			'highlight_series' => 0,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s Total Offers Accepted in this period', 'woofunnels-upstroke-reports' ), '<strong>' . $data->total_offers_accepted . '</strong>' ),
			'color'            => $this->chart_colours['offers_accepted'],
			'placeholder'      => __( 'Total offers accepted.', 'woofunnels-upstroke-reports' ),
			'highlight_series' => 2,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s Total Offers Rejected in this period', 'woofunnels-upstroke-reports' ), '<strong>' . $data->total_offers_declined . '</strong>' ),
			'color'            => $this->chart_colours['offers_declined'],
			'placeholder'      => __( 'Total offers desclined.', 'woofunnels-upstroke-reports' ),
			'highlight_series' => 1,
		);

		$legend[] = array(
			'title'            => sprintf( __( '%s Overall conversion rate', 'woofunnels-upstroke-reports' ), '<strong>' . $data->overall_conversion_rate . '%</strong>' ),
			'color'            => $this->chart_colours['conversion_rate'],
			'placeholder'      => __( 'Overall conversion rate.', 'woofunnels-upstroke-reports' ),
			'highlight_series' => '5',
		);
	...
	public function output_report() {
		...
		$this->chart_colours = array(
			'grass_upsells'         => '#4D94D9',
			'average_grass_upsells' => '#f1c40f',
	...
	public function get_export_button() {
        ...
        <a
                ...
                class="export_csv"
                data-export="chart"
                data-xaxes="<?php esc_attr_e( 'Date', 'woofunnels-upstroke-reports' ); ?>"
                data-exclude_series=""
        ...
        <script type="text/javascript">
            ...
            jQuery(function () {
                ...
                var drawGraph = function (highlight) {
                    var series = [
                        {
                            label: "<?php echo esc_js( __( 'Funnels Triggered', 'woofunnels-upstroke-reports' ) ); ?>",
                            data: upsells_data.funnels_triggered,
                            color: '<?php echo $this->chart_colours['funnels_triggered']; ?>',
                            bars: {
                                fillColor: '<?php echo $this->chart_colours['funnels_triggered']; ?>',
                                fill: true,
                                show: true,
                                lineWidth: 6,
                                order: 0,
                        ...
                        {
                            label: "<?php echo esc_js( __( 'Offers Declined', 'woofunnels-upstroke-reports' ) ); ?>",
                            data: upsells_data.offers_declined,
                            color: '<?php echo $this->chart_colours['offers_declined']; ?>',
                            bars: {
                                fillColor: '<?php echo $this->chart_colours['offers_declined']; ?>',
                                fill: true,
                                order: 1,
                                show: true,
                                lineWidth: 5,
                        ...
                        {
                            label: "<?php echo esc_js( __( 'Offers Accepted', 'woofunnels-upstroke-reports' ) ); ?>",
                            data: upsells_data.offers_accepted,
                            color: '<?php echo $this->chart_colours['offers_accepted']; ?>',
                            bars: {
                                fillColor: '<?php echo $this->chart_colours['offers_accepted']; ?>',
                                fill: true,
                                show: true,
                                order: 2,
                                lineWidth: 3,
                        ...
                        {
                            label: "<?php echo esc_js( __( 'Upsells', 'woofunnels-upstroke-reports' ) ); ?>",
                            data: upsells_data.upsells,
                            yaxis: 2,
                            color: '<?php echo $this->chart_colours['grass_upsells']; ?>',
                        ...
                        {
                            label: "<?php echo esc_js( __( 'Average gross upsells', 'woofunnels-upstroke-reports' ) ); ?>",
                            data: [[ <?php echo min( array_keys( $upsells ) ); ?>, <?php echo $this->report_data->average_grass_upsells; ?> ], [ <?php echo max( array_keys( $upsells ) ); ?>, <?php echo $this->report_data->average_grass_upsells; ?> ]],
                            yaxis: 2,
                            color: '<?php echo $this->chart_colours['average_grass_upsells']; ?>',
                            points: {show: false},
